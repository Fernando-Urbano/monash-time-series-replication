...
# Navigate up two levels to get the parent directory of the parent directory
# base_dir <- normalizePath(file.path(dirname(dirname(current_file_path))))

BASE_DIR <- normalizePath(file.path((current_file_path)))
chosen_models_path <- file.path(BASE_DIR, "src", "chosen_models.txt")
chosen_models <- readLines(chosen_models_path)
chosen_datasets_path <- file.path(BASE_DIR, "src", "chosen_datasets.txt")
chosen_datasets <- readLines(chosen_datasets_path)
...
if ('tbats' %in% chosen_models || 'all' %in% chosen_models){
    ...
    if ('hospital_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
        do_fixed_horizon_local_forecasting("hospital", "tbats", "hospital_dataset.tsf", "series_name", "start_timestamp", 12, TRUE)
    }
    if ('fred_md_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
...
if ('tbats' %in% chosen_models || 'all' %in% chosen_models){
    ...
    if ('saugeenday_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
        do_fixed_horizon_local_forecasting("saugeen_river_flow", "tbats", "saugeenday_dataset.tsf", "series_name", "start_timestamp", 30)
    }
    if ('sunspot_dataset_without_missing_values' %in% chosen_datasets || 'all' %in% chosen_datasets){
    ...
    if ('covid_deaths_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
        do_fixed_horizon_local_forecasting("covid_deaths", "tbats", "covid_deaths_dataset.tsf", "series_name", "start_timestamp", 30, TRUE)
    }
    if ('weather_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
        do_fixed_horizon_local_forecasting("weather", "tbats", "weather_dataset.tsf", NULL, NULL, 30)
    }
    if ('bitcoin_dataset_without_missing_values' %in% chosen_datasets || 'all' %in% chosen_datasets){
    ...
    if ('vehicle_trips_dataset_without_missing_values' %in% chosen_datasets || 'all' %in% chosen_datasets){
    ...
    if ('temperature_rain_dataset_without_missing_values' %in% chosen_datasets || 'all' %in% chosen_datasets){
...
if ('tbats' %in% chosen_models || 'all' %in% chosen_models){
    ...
    if ('electricity_hourly_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
    ...
    if ('solar_10_minutes_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
        do_fixed_horizon_local_forecasting("solar_10_minutes", "tbats", "solar_10_minutes_dataset.tsf", "series_name", "start_timestamp", 1008)
    }
    if ('kdd_cup_2018_dataset_without_missing_values' %in% chosen_datasets || 'all' %in% chosen_datasets){
    ...
    if ('pedestrian_counts_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
    ...
    if ('australian_electricity_demand_dataset' %in% chosen_datasets || 'all' %in% chosen_datasets){
    ...
    if ('rideshare_dataset_without_missing_values' %in% chosen_datasets || 'all' %in% chosen_datasets){
